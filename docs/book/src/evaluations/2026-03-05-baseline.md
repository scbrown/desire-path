...
**Recommendation**: Bobbin could inject directory tree output on EISDIR errors (tracked internally). `tree` command now installed on the eval hosts.
...
No alerts fired. Tracked internally: add monitoring.
...
| just | Not installed | True positive — `just` (justfile runner) not on the eval host |
...
2. **dp map** — Map documentation to failing tool patterns
3. **Struggling detection** — Identify agents retrying same failure pattern
4. **Recovery tracking** — Detect when fixes reduce failures
...
- Did MCP monitoring reduce downtime?
