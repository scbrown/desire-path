...
links, and links of the form https://github.com/scbrown/desire-path/(blob|tree)/main/<path>,
...
Usage: check-md-links.py [FILE.md ...]   (no arguments: README and book Markdown)
"""
...
REPO_URL = re.compile(r"^https://github\.com/scbrown/desire-path/(?:blob|tree)/main/(.+)$")
...
def main(argv):
    ...
    files = argv or ["README.md", *sorted(str(p.relative_to(root))
        for p in Path(root, "docs/book/src").rglob("*.md"))]
    broken = []
    for name in files:
        path = os.path.join(root, name)
        if not os.path.isfile(path):
            broken.append(f"{name}: missing source file")
            continue
