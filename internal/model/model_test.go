...
func TestInvocationOmitsEmptyFields(t *testing.T) {
	...
	// is_error is NOT omitempty — it always appears in JSON output.
	for _, key := range []string{"instance_id", "host_id", "error", "cwd", "metadata"} {
		if _, ok := m[key]; ok {
			t.Errorf("expected %q to be omitted for zero value", key)
		}
	}
	// Verify is_error is present even when false (no omitempty).
	if _, ok := m["is_error"]; !ok {
		t.Error("expected is_error to be present (no omitempty tag)")
	}
}
