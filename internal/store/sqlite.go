...
func (s *SQLiteStore) ListInvocations(ctx context.Context, opts InvocationListOpts) ([]model.Invocation, error) {
	...
	if opts.IsError != nil {
		query += " AND is_error = ?"
		args = append(args, boolToInt(*opts.IsError))
	}
	...
	for rows.Next() {
		...
		var source, instanceID, hostID, errStr, cwd, ts, metadata sql.NullString
		var isError int
		if err := rows.Scan(&inv.ID, &source, &instanceID, &hostID, &inv.ToolName, &isError, &errStr, &cwd, &ts, &metadata); err != nil {
			return nil, fmt.Errorf("scan invocation: %w", err)
		}
		inv.Source = source.String
...
func (s *SQLiteStore) InvocationStats(ctx context.Context) (InvocationStatsResult, error) {
	...
	if err := s.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM invocations WHERE is_error = 1").Scan(&st.Errors); err != nil {
		return st, fmt.Errorf("count errors: %w", err)
	}
	...
	for toolRows.Next() {
		...
		if err := toolRows.Scan(&nc.Name, &nc.Count); err != nil {
			return st, fmt.Errorf("scan top tool: %w", err)
		}
	...
	srcRows, err := s.db.QueryContext(ctx,
		"SELECT source, COUNT(*) AS cnt FROM invocations WHERE source IS NOT NULL AND source != '' GROUP BY source ORDER BY cnt DESC LIMIT 5")
	if err != nil {
	...
	for srcRows.Next() {
		...
		if err := srcRows.Scan(&nc.Name, &nc.Count); err != nil {
			return st, fmt.Errorf("scan top source: %w", err)
		}
