...
func TestMigrateV5TurnColumns(t *testing.T) {
	...
	// V5 migration should have added turn columns.
	inv := model.Invocation{
		ID:           "v5-1",
		Source:       "cc",
		ToolName:     "Read",
	...
	if err := s.RecordInvocation(ctx, inv); err != nil {
		t.Fatalf("RecordInvocation with turn columns: %v", err)
	}
	...
	if got[0].TurnID != "sess:0" {
		t.Errorf("TurnID: got %q, want %q", got[0].TurnID, "sess:0")
	}
