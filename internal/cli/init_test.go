...
func TestRunInitUnknownSource(t *testing.T) {
	err := runInit("nonexistent-source", false, "", nil)
	if err == nil {
...
func TestRunInitSourceWithoutInstaller(t *testing.T) {
	...
	err := runInit("test-no-installer-init", false, "", nil)
	if err == nil {
...
func TestRunInitAlreadyInstalled(t *testing.T) {
	...
	go func() {
		errCh <- runInit("test-already-installed", false, "", nil)
		w.Close()
	}()
...
func TestRunInitAlreadyInstalledJSON(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- runInit("test-already-installed-json", false, "", nil)
		w.Close()
	}()
...
func TestRunInitNotYetInstalled(t *testing.T) {
	...
	errCh := make(chan error, 1)
	go func() {
		errCh <- runInit("test-not-yet-installed", false, "", nil)
		w.Close()
	}()
